...
import { motion } from 'framer-motion'; //smooth page transitions

const Login = () => {
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')
    const location = useLocation()
    const [successmssg, setsuccessMssg] = useState(location.state?.message || '')
            ...
            >
                {successmssg && (
                    <div className="absolute top-5 left-1/2 transform -translate-x-1/2">
                        <p className="bg-green-500 text-white px-4 py-2 rounded shadow-md font-medium">
                            {successmssg}
                        </p>
                    </div>
                )}
                ...
                <div className='flex items-center justify-center h-screen bg-gray-100  dark:bg-gray-900 dark:text-white transition-all'>
                    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 dark:text-white text-gray-500 max-w-[340px] w-full mx-4 md:p-6 p-4 py-8 text-left text-sm rounded-xl shadow-[0px_0px_10px_0px] shadow-black/10">

                        <h2 className="text-2xl font-bold mb-9 text-center text-blue-500">Login to your Account!!</h2>
                        ...
                        <div className="flex items-center my-2 border bg-indigo-500/5 border-gray-500/10 rounded gap-1 pl-2">
                            ...
                            <input
                                className="w-full outline-none bg-transparent px-1.5 py-2.5"
                                type="email"
                                placeholder="Email"
                                required
                                value={email}
                        ...
                        <div className="flex items-center mt-2 mb-4 border bg-indigo-500/5 border-gray-500/10 rounded gap-1 pl-2">
                            ...
                            <input
                                className="w-full outline-none bg-transparent px-2.5 py-2.5"
                                type="password"
